...
export default function AdminDraws() {
  ...
  const load = async () => {
    try {
      ...
      setMsg({ type: "error", text: "Failed to load draws." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSimulate = async (id) => {
    ...
    try {
      ...
      setMsg({
        type: "error",
        text: err.response?.data?.error || "Simulation failed.",
      });
    } finally {
      setActionId(null);
    }
  ...
  const handleRun = async (id) => {
    ...
    try {
      ...
      setMsg({
        type: "success",
        text: `Draw run! 5-match: ${res.winners?.winners5?.length || 0}, 4-match: ${res.winners?.winners4?.length || 0}, 3-match: ${res.winners?.winners3?.length || 0}`,
      });
      await load();
    } catch (err) {
      setMsg({
        type: "error",
        text: err.response?.data?.error || "Failed to run draw.",
      });
    } finally {
      setActionId(null);
    }
  ...
  return (
    <div style={{ minHeight: "100vh" }}>
      ...
      <div className="container section">
        <h1 className="page-title" style={{ fontSize: 28, marginBottom: 28 }}>
          Draw Management
        </h1>
        ...
        {simulation && (
          <div
            className="card"
            style={{ marginBottom: 24, borderColor: "var(--blue)" }}
          >
            <h3
              className="section-title"
            ...
            <p className="text-sm text-muted" style={{ marginBottom: 12 }}>
              ...
              <strong style={{ color: "var(--accent)" }}>
                {simulation.drawNumbers?.join(", ")}
              </strong>
            </p>
            <div className="grid-3">
              {[
                ["5-Match 🏆", simulation.preview5],
                ["4-Match 🥈", simulation.preview4],
                ["3-Match 🥉", simulation.preview3],
              ].map(([label, names]) => (
                <div key={label}>
                  ...
                  {names?.length ? (
                    ...
                    <div className="text-sm text-muted">No winners</div>
                  )}
                </div>
              ))}
            </div>
                ...
                <tbody>
                  {draws.map((d) => (
                    <tr key={d.id}>
                      ...
                      <td>
                        <span
                          className={`badge ${statusColor[d.status] || "badge-gray"}`}
                        >
                          {d.status}
                        </span>
                      </td>
