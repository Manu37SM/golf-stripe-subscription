...
export default function AdminWinners() {
  ...
  const load = async () => {
    try {
      ...
      setMsg({ type: "error", text: "Failed to load winners." });
    } finally {
      setLoading(false);
    }
  };
                ...
                <tbody>
                  {winners.map((w) => (
                    <tr key={w.id}>
                      ...
                      <td>
                        <span
                          className={`badge ${statusColor[w.status] || "badge-gray"}`}
                        >
                          {w.status}
                        </span>
                      </td>
                      <td>
                        {w.proof_url ? (
                          <a
                            href={`https://golf-stripe-subscription.onrender.com/${w.proof_url}`}
                            target="_blank"
                            rel="noreferrer"
