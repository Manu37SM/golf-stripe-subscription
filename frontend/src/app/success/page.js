...
export default function Success() {
  ...
  useEffect(() => {
    ...
    const activate = async () => {
      try {
        ...
        console.warn("activate-session:", err.response?.data?.error || err.message);
        setStatus("done");
      }
    };
  ...
  return (
    <div style={{
      ...
      background: "radial-gradient(ellipse 60% 40% at 50% 0%, rgba(127,255,110,0.08) 0%, transparent 70%), var(--bg)",
      padding: 24,
    }}>
      <div style={{ textAlign: "center", maxWidth: 480 }}>
        {status === "activating" ? (
          <>
            ...
            <p style={{ color: "var(--text2)", fontSize: 14 }}>Just a moment, please don't close this page.</p>
          </>
        ) : (
          <>
            ...
            <p style={{ color: "var(--text2)", marginBottom: 32, lineHeight: 1.6 }}>
              Your subscription is now active. You can start entering Stableford scores,
              join monthly prize draws, and support your chosen charity.
            </p>
            <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
              <Link href="/dashboard" className="btn btn-primary" style={{ fontSize: 15, padding: "13px 28px" }}>
                Go to Dashboard →
              </Link>
              <Link href="/scores" className="btn btn-outline" style={{ fontSize: 15, padding: "13px 28px" }}>
                Add Scores
              </Link>
            </div>
          </>
